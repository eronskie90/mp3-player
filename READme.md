# MP3 Player

A desktop MP3 player built with Python, Tkinter, Pygame, and Pillow.

## Features

* Play and pause music
* Resume playback
* Stop music
* Next track
* Add songs to the playlist
* Volume control
* Progress tracking
* Album cover display
* Playlist management
* Automatic music and album folder handling

## Technologies Used

* Python
* Tkinter
* Pygame
* Pillow

## Project Structure

```text
mp3-player/
├── playlist.py
├── requirements.txt
├── .gitignore
├── album/
│   └── .gitkeep
└── music/
    └── .gitkeep
```

## Installation

1. Clone this repository.

2. Install the required packages:

```bash
pip install -r requirements.txt
```

3. Add your own music files to the `music` folder.

4. Add your own album cover images to the `album` folder.

5. Run the application:

```bash
python playlist.py
```

## Notes

The repository does not include sample MP3, MP4, or album artwork files. Users can add their own media files to the `music` and `album` folders.

## Purpose

This project was created as a Python desktop application project to practice GUI development, audio playback, playlist management, file handling, and third-party Python libraries.

````

Save → close Notepad.

**Don't commit yet.** After saving, run:

```cmd
type README.md
````

Send me what appears. Then iche-check natin bago natin i-push.
